...
class CafeController {
    ...
    private val shelterToCat = mutableMapOf(ShelterOne to shelterOneCats.toMutableSet(), ShelterTwo to shelterTwoCats.toMutableSet())
    ...
    fun sellItems(items: List<Product>, customerId: String) {
        ...
        println("Customer with ID #${receipt.customerId} just purchased ${receipt.products.map { it.name }} costing ${receipt.totalPrice} dollars in total")
    }
    ...
    fun getNumberOfAdoptionsPerShelter(): Map<String, Int> {
        ...
        return shelters.map { shelter -> shelter.name to adoptedCats.count { cat -> cat.shelterId == shelter.shelter_id } }.toMap()
    }
